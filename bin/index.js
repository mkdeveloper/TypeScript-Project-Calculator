...
async function welcome() {
    const welcomeMessage = chalkAnimation.rainbow(`Welcome To Muhammad Khubaib's Calculator\n`);
    await sleep();
    welcomeMessage.stop();
}
await welcome();
...
do {
    ...
    if (!isNaN(answer.firstNumber) && !isNaN(answer.secondNumber)) {
        ...
        if (answer.operator === "x Raise to the Power y") {
            console.log(chalk.greenBright(`${answer.firstNumber} raise to the power ${answer.secondNumber} is: ${result}`));
        }
        else {
