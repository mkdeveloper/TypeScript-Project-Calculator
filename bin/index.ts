...
const sleep = (ms = 2000) => new Promise<void>((r) => setTimeout(r, ms));

async function welcome() {
  const welcomeMessage = chalkAnimation.rainbow(
    `Welcome To Muhammad Khubaib's Calculator\n`
  );
...
function calculation(opt: string, numFirst: number, numSecond: number): number {
  switch (opt) {
    case "Addition":
